...
pub fn update(transaction: &DatabaseTransaction<'_, RW>, meta: &PackageMeta) -> Result<(), DatabaseError> {
    let found =
        exists(transaction, &meta.name, &meta.arch, meta.arch_sub.as_deref())?.ok_or(DatabaseError::PackageNotFound)?;

    let bytes = rmp_serde::to_vec(meta)?;
    transaction.put(Store::Packages, found, &bytes)?;
...
pub fn exists<K: TransactionKind>(
    ...
    for (uuid, value) in transaction.entries(Store::Packages)? {
        ...
        if meta.name == name
            ...
            && meta.name == name
            && meta.arch == arch
            && meta.arch_sub.as_deref() == arch_sub
        {
            return Ok(Some(uuid));
        }
    }
