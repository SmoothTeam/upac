...
- `errors` / `lock` — split out of `types` into their own top-level public modules: `CommonError` (wraps `HookError`/`DecoderError`/`RepoError`/`DatabaseError`/`SysrootError`/`LockError`/`DeployRecordError` — all of them public too now, one under each module listed above) and `Lock`/`LockError` (the exclusive system lock, §5.9). Made public because external code implementing `E: From<CommonError>` to build its own pipeline via `orchestrator` under static linking needs to name `CommonError`.
