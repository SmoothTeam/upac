...
***Ещё не начато:*** `config_merge` (3-way слияние `/etc`, §5.1), `boot` (загрузочной записи, разовый вход/подтверждение/откат, см. **§5.2**; берёт `composefs-boot`, grub через `blscfg` — отдельных плагинов нет, только BLS-совместимые загрузчики by desing), и само построение графа зависимостей пакетов на стороне `lib` (decoder сейчас только отдаёт сырой список зависимостей пакета через `decode` — граф ещё никто не обходит, да и сетевого слоя для скачивания пакетов тоже нет).
