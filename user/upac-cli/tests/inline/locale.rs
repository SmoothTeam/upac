...
fn single_language_loader(language: &str) -> FluentLanguageLoader {
    ...
    let loader = FluentLanguageLoader::new("upac-cli", language.clone());
...
fn a_missing_required_flag_is_reported_in_english() {
    let message = parse_error::<English>(&["up", "file", "add", "notes.txt", "--package", "upac"]);

    assert!(
        message.starts_with("error: the following required arguments were not provided: --arch <ARCH>"),
        "{message}"
    );
}
...
fn an_unknown_subcommand_is_reported_in_russian() {
    let message = parse_error::<Russian>(&["up", "pkg", "instal"]);

    assert!(
        message.starts_with("ошибка: неизвестная подкоманда 'instal'"),
        "{message}"
    );
}
