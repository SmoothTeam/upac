...
pub struct InsertEntryStage;

impl Stage<PartitionError> for InsertEntryStage {
    fn run(
        ...
        let requested = ctx_get!(context, RequestedPartition);

        let mut device = OpenOptions::new().read(true).write(true).open(&**device_path)?;

        let mut table = GptTable::open(&mut device)?;
        let number = table.insert(requested.kind, &requested.label, requested.size_mib)?;
        table.write_into(&mut device)?;

        context.put(InsertedPartitionNumber(number));
