...
pub fn getRefBody(machine: *DiffMachine, ref: [*:0]const u8) DiffError![]const u8 {
    ...
    const repo = machine.repo orelse return DiffError.RepoOpenFailed;

    if (c_libs.ostree_repo_resolve_rev(repo, ref, 1, &commit_checksum, &machine.gerror) == 0) return DiffError.CommitNotFound;

    if (c_libs.ostree_repo_load_variant(repo, c_libs.OSTREE_OBJECT_TYPE_COMMIT, commit_checksum, &commit_variant, &machine.gerror) == 0) return DiffError.CommitNotFound;
    ...
    return machine.allocator.dupe(u8, ptr[0..len]) catch |err| return err;
}
...
pub fn resolveCommitRoot(machine: *DiffMachine, ref: [*:0]const u8) DiffError!*c_libs.GFile {
    ...
    if (c_libs.ostree_repo_resolve_rev(repo, ref, 0, &commit_checksum, &machine.gerror) == 0) return DiffError.CommitNotFound;

    if (c_libs.ostree_repo_read_commit(repo, commit_checksum, &root_gfile, null, machine.cancellable, &machine.gerror) == 0) return DiffError.CommitNotFound;
...
pub fn buildFilePkgMap(machine: *DiffMachine, ref: [*:0]const u8, out: *std.StringHashMap([]const u8)) DiffError!void {
    const body = try getRefBody(machine, ref);
    ...
    while (iter.next()) |entry| {
        ...
        var file_map = readFiles(abs_db_path, entry.value_ptr.*, machine.allocator) catch continue;
        defer freeFileMap(&file_map, machine.allocator);
...
pub fn collectEntries(arr: *c_libs.GPtrArray, root_gfile: *c_libs.GFile, kind: DiffKind, use_target: bool, result: *std.ArrayList(RawDiffEntry), allocator: std.mem.Allocator) DiffError!void {
    var index: usize = 0;
    while (index < arr.*.len) : (index += 1) {
        ...
        const path_dupe = allocator.dupe(u8, std.mem.span(relative_path)) catch |err| return err;
        result.append(allocator, .{ .path = path_dupe, .kind = kind }) catch |err| return err;
    }
}
