...
pub fn install(install_request_c: CInstallRequest) callconv(.c) i32 {
    ...
    const install_data = installer_module.InstallData{ .packages = install_entries, .branch = install_request_c.branch.asZ(), .repo_path = install_request_c.repo_path.asZ(), .root_path = install_request_c.root_path.asZ(), .on_progress = if (install_request_c.on_progress) |cb| @as(InstallProgressFn, @ptrCast(cb)) else null, .progress_ctx = install_request_c.progress_ctx, .max_retries = install_request_c.max_retries, .cancel_token = install_request_c.cancel_token orelse return @intFromEnum(fromError(error.InvalidEntry, Operation.install)) };
