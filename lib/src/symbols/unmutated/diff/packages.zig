...
const DiffMachine = @import("upac-diff-packages").DiffMachine;

pub fn diff_packages(request_c: CUnmutatedRequest, out_c: *CUnmutatedResponse) callconv(.c) i32 {
    ...
    out_c.diff_packages = .{ .ptr = diff_entries.ptr, .len = diff_entries.len };
