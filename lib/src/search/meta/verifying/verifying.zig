...
pub const VerifyingMachine = struct {
    ...
    fn stateFailed(self: *VerifyingMachine, err: SearchMetaError) SearchMetaError {
        if (self.base) |*base| {
            base.close();
            self.base = null;
        }
...
pub fn run(machine: *SearchMetaMachine) SearchMetaError!void {
    ...
    while (state != .done) {
        if (machine.data.cancel_token.isCancelled()) return verifying_machine.stateFailed(SearchMetaError.Cancelled);
        state = switch (state) {
...
// ── States ────────────────────────────────────────────────────────────────────
fn stateCheckRoot(machine: *VerifyingMachine) SearchMetaError!VerifyingState {
    const root_path = std.mem.span(machine.searcher.data.root_path);

    std.Io.Dir.accessAbsolute(machine.searcher.io, root_path, .{}) catch return SearchMetaError.PathNotFound;

    return .check_prefix;
}

fn stateCheckPrefix(machine: *VerifyingMachine) SearchMetaError!VerifyingState {
    const root_path = std.mem.span(machine.searcher.data.root_path);

    const prefix_path = std.fs.path.joinZ(machine.searcher.allocator, &.{ root_path, PREFIX }) catch return SearchMetaError.AllocZFailed;
    defer machine.searcher.allocator.free(prefix_path);

    std.Io.Dir.accessAbsolute(machine.searcher.io, prefix_path, .{}) catch return SearchMetaError.PathNotFound;

    return .check_database;
}

fn stateCheckDatabase(machine: *VerifyingMachine) SearchMetaError!VerifyingState {
    const root_path = std.mem.span(machine.searcher.data.root_path);
    ...
    std.Io.Dir.accessAbsolute(machine.searcher.io, database_file_path, .{}) catch return SearchMetaError.PathNotFound;

    return .open_database;
}

fn stateOpenDatabase(machine: *VerifyingMachine) SearchMetaError!VerifyingState {
    const root_path = std.mem.span(machine.searcher.data.root_path);
