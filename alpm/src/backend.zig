...
pub const BackendMachine = struct {
    ...
    pub fn report(self: *BackendMachine, event: StateId) void {
        ...
        cb(event, CSlice.fromSlice(std.fs.path.basename(std.mem.span(self.request.package_path_c))), self.request.progress_ctx);
    }
...
const CPrepareRequest = extern struct {
    ...
    pub fn validate(req: CPrepareRequest) !void {
        ...
        try req.pacakge_path.validate();
        try req.temp_dir_path.validate();
...
pub export fn prepare(request_c: *const CPrepareRequest, out_meta: **CPackageMeta, out_temp_path: *CSlice) callconv(.c) i32 {
    ...
    const zig_request = PrepareRequest{
        .package_path_c = request_c.pacakge_path.asZ(),
        .temp_dir_path_c = request_c.temp_dir_path.asZ(),
