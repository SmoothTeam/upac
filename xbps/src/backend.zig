...
// Parameters for the package preparation request
...
// Listing specific backend errors
...
pub const BackendMachine = struct {
    ...
    pub fn report(self: *BackendMachine, event: StateId) void {
        ...
        cb(event, CSlice.fromSlice(std.fs.path.basename(std.mem.span(self.request.package_path))), self.request.progress_ctx);
    }
