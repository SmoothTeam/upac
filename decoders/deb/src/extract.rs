...
impl ExtractedMetadata {
    pub fn extract(package_path: &str, output_dir: &str, cancel: &CancelToken) -> Result<Self, DecodeError> {
        ...
        let (control_name, control_bytes) = control_tar.ok_or(DecodeError::MissingMetadata)?;
        let (data_name, data_bytes) = data_tar.ok_or(DecodeError::MissingMetadata)?;

        let (control, scripts_present) = Self::extract_control(control_bytes, &control_name, cancel)?;
        let license = Self::extract_data(data_bytes, &data_name, output_dir, cancel)?;
    ...
    fn extract_control(
        ...
        let reader = Self::open_tar_reader(member_name, bytes)?;
        ...
        for entry in archive.entries()? {
            ...
            if normalized == CONTROL_ENTRY {
                control = Some(read_to_string(&mut entry)?);
        ...
        for entry in archive.entries()? {
                ...
                && normalized.ends_with(COPYRIGHT_ENTRY_SUFFIX)
            {
                ...
                license = String::from_utf8(bytes)
                    .ok()
                    .and_then(|content| Self::parse_license_from_copyright(&content));
                continue;
            }
    ...
    fn open_tar_reader(member_name: &str, bytes: Vec<u8>) -> Result<Box<dyn Read>, DecodeError> {
        ...
        match member_name {
            _ if member_name.ends_with(".zst") => Ok(Box::new(ZstdDecoder::new(cursor)?)),
            _ if member_name.ends_with(".xz") => Ok(Box::new(XzDecoder::new(cursor))),
            _ if member_name.ends_with(".gz") => Ok(Box::new(GzDecoder::new(cursor))),
            CONTROL_TAR_PREFIX | DATA_TAR_PREFIX => Ok(Box::new(cursor)),
