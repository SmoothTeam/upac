...
pub export fn prepare(request_c: *const CPrepareRequest, out_meta: **CPackageMeta, out_temp_path: *CSlice) callconv(.c) i32 {
    ...
    const prepare_data = PrepareData{
        ...
        .temp_path_c = request_c.temp_dir_path.asZ(),
    ...
    out_meta_ptr.* = CPackageMeta{
        ...
        .maintainer = dupeToCSlice(std.heap.c_allocator, result.meta.maintainer) catch return @intFromEnum(fromError(BackendError.AllocZFailed)),
        ...
        .license = CSlice.fromSlice(if (result.meta.license) |lic|
            std.heap.c_allocator.dupeZ(u8, lic) catch return @intFromEnum(BackendErrorCode.alloc_failed)
        else
            null),
        .url = CSlice.fromSlice(if (result.meta.url) |url|
            std.heap.c_allocator.dupeZ(u8, url) catch return @intFromEnum(BackendErrorCode.alloc_failed)
        else
            null),
        .sha256 = result.meta.sha256,
        .installed_size = result.meta.installed_size,
    };
