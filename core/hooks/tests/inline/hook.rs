...
use super::load_hooks;
use super::pipeline::{Operation, PipelineTrigger, Timing};
use super::primitive::{ExecutedSteps, Step};
...
fn executed_steps_rollback_guard_unwinds_in_reverse_order() {
    ...
    let mut executed = ExecutedSteps::default();
    for mut step in hook_file.steps.drain(..) {
        step.execute().unwrap();
        executed.0.push(step);
    }
    assert!(c.exists());
...
fn hook_file_run_executes_all_steps_and_returns_advance() {
    ...
    let (_, result, _guard) =
        ConcurrentStage::<HookError>::run(Box::new(hook_file), ProgressEventBuilder::new(0)).unwrap();
...
fn hook_file_run_rolls_back_and_errors_when_a_critical_step_fails() {
    ...
    let result = ConcurrentStage::<HookError>::run(Box::new(hook_file), ProgressEventBuilder::new(0));
