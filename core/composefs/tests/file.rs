...
#[test]
fn import_directory_merges_into_an_existing_directory_instead_of_replacing_it() {
    ...
    let (_scratch, repository) = open_repository("merge-repo");
    let mut tree = empty_tree();
    let mut ctx = ImportContext::default();
    let cancel = CancelToken::new();

    for source in [&first_source, &second_source] {
        FileHandle::new(PathBuf::new())
            .import_directory(&repository, &mut tree, source.path(), &mut ctx, &cancel, &mut |_| {})
            .unwrap();
    }

    assert_eq!(
        FileHandle::new("lib/modules/7.0/vmlinuz")
            .read_file(&repository, &tree)
            .unwrap(),
        b"kernel"
    );
    assert_eq!(
        FileHandle::new("lib/libupac.so").read_file(&repository, &tree).unwrap(),
        b"library"
    );
}

#[test]
fn import_directory_replaces_a_file_that_stands_where_a_directory_is_imported() {
    ...
    let (_scratch, repository) = open_repository("replace-leaf-repo");
    let mut tree = empty_tree();
    let mut ctx = ImportContext::default();
    let cancel = CancelToken::new();

    FileHandle::new("conflict")
        .insert_file(
            &repository,
            &mut tree,
    ...
    assert_eq!(
        FileHandle::new("conflict/inner.txt")
            .read_file(&repository, &tree)
            .unwrap(),
        b"inner"
    );
